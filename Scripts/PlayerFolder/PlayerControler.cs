...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var dashInput = Input.GetKeyDown(KeyCode.LeftShift); // 시프트 키로 대시
        var jumpInput = Input.GetKeyDown(KeyCode.Space); // 스페이스바 누르면 점프 두 번 누르면 더블 점프
        ...
        if (inputX != 0)
        {
            transform.localScale = new Vector3(Mathf.Sign(inputX) * 9, 9, 1);
        }
